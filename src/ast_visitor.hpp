...
template <class T, class S> class BaseASTVisitor {
  ...
  virtual T visitImplStatement(ImplStatement &node, const S &state);
  ...
  virtual T visitImplSelect(ImplSelect &node, const S &state);
  virtual T visitSizeof(Sizeof &node, const S &state);
...
T BaseASTVisitor<T, S>::visitStatement(Statement &node, const S &state) {
  ...
  } else if (dynamic_cast<ImplStatement *>(&node) != nullptr) {
    return visitImplStatement(dynamic_cast<ImplStatement &>(node), state);
...
T BaseASTVisitor<T, S>::visitExpression(Expression &node, const S &state) {
  ...
  } else if (dynamic_cast<Sizeof *>(&node) != nullptr) {
    return visitSizeof(dynamic_cast<Sizeof &>(node), state);
  } else if (dynamic_cast<ImplSelect *>(&node) != nullptr) {
    return visitImplSelect(dynamic_cast<ImplSelect &>(node), state);
  }
...
template <class T, class S>
T BaseASTVisitor<T, S>::visitSizeof(Sizeof &node, const S &state) {
  return std::move(defaultValue);
}

template <class T, class S>
T BaseASTVisitor<T, S>::visitImplStatement(ImplStatement &node,
                                           const S &state) {
  return std::move(defaultValue);
}

template <class T, class S>
T BaseASTVisitor<T, S>::visitImplSelect(ImplSelect &node, const S &state) {
  return std::move(defaultValue);
}
