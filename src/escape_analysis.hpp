...
public:
  ...
  TraceFlowVisitedState() : visitedFroms(), visitedSpecialFroms(), visitedFlows(){};
};
...
public:
  ...
  EscapeAnalysisState(bool in_func, FlowList *curFlowList) : in_func(in_func), curFlowList(curFlowList){};
...
class EscapeAnalysisPass : public BaseIRVisitor<int, EscapeAnalysisState> {
  ...
  int visitFunctionDeclare(FunctionDeclare &instruct, const EscapeAnalysisState &state) override;
  int visitTupleLiteral(TupleLiteral &instruct, const EscapeAnalysisState &state) override;
  int visitFunctionCall(FunctionCall &instruct, const EscapeAnalysisState &state) override;
  int visitAllocation(Allocation &instruct, const EscapeAnalysisState &state) override;
  int visitAddress(Address &instruct, const EscapeAnalysisState &state) override;
  int visitBuiltinCast(BuiltinCast &instruct, const EscapeAnalysisState &state) override;
  ...
  int visitBasicBlock(BasicBlock &instruct, const EscapeAnalysisState &state) override;
  int visitForwardIdentifier(ForwardIdentifier &instruct, const EscapeAnalysisState &state) override;

  static bool argsContain(const std::vector<std::reference_wrapper<Allocation>> &funcArgs, const Expression &expr);

  void calculateFunctionFlowMetadata(ir::FuncFlowList &functionFlows,
  ...
  void markEscapingAllocations(const ir::FlowList &flows, const BasicBlockList &blocks);

public:
  EscapeAnalysisPass(bool print_flows, bool print_escapes, bool print_func_flow_metadata, std::ostream &output)
      : BaseIRVisitor(0), print_flows(print_flows), print_func_flow_metadata(print_func_flow_metadata),
