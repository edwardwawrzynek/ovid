...
bool operator==(const FlowValue &lhs, const FlowValue &rhs) {
  return lhs.expr.val.id == rhs.expr.val.id && lhs.indirect_level == rhs.indirect_level &&
         lhs.field_selects == rhs.field_selects;
}

bool operator!=(const FlowValue &lhs, const FlowValue &rhs) { return !(lhs == rhs); }

bool operator==(const Flow &lhs, const Flow &rhs) { return lhs.from == rhs.from && lhs.into == rhs.into; }

bool operator!=(const Flow &lhs, const Flow &rhs) { return !(lhs == rhs); }

FlowValue::FlowValue(Expression &expr, int32_t indirect_level, const std::vector<std::vector<int32_t>> &field_selects,
                     EscapeType is_escape)
    : expr(expr), indirect_level(indirect_level), field_selects(field_selects), is_escape(is_escape) {
...
const ast::Type *FlowValue::applyFieldSelectToType(const ast::Type *type, const std::vector<int32_t> &field_select) {
...
FlowValue::getFlowingTypesFromType(const ast::Type *exprType, int32_t indirect_level,
  ...
  type = applyFieldSelectToType(type, field_selects[field_selects.size() - indirect_level - 1]);
  ...
  if (indirect_level == 0) {
    ...
    if (pointerType != nullptr) {
      auto derefedType =
          getFlowingTypesFromType(pointerType->type->withoutMutability(), indirect_level - 1, field_selects);
      ...
      for (auto &compType : componentTypes) {
        ...
        auto pointerCompType = dynamic_cast<const ast::PointerType *>(compType->withoutMutability());
        if (pointerCompType != nullptr) {
          auto derefedType =
              getFlowingTypesFromType(pointerCompType->type->withoutMutability(), indirect_level - 1, field_selects);
...
std::vector<const ast::Type *> FlowValue::getFlowingTypes() const {
  return getFlowingTypesFromType(expr.type.get(), indirect_level, field_selects);
}
...
FlowValue FlowValue::getFlowFromExpressionWithoutCopy(Expression &expr, std::vector<int32_t> &field_selects_on_deref) {
  std::vector<int32_t> retroactive_field_selects;
  auto fieldSelect = dynamic_cast<FieldSelect *>(&expr);
  if (fieldSelect != nullptr) {
    auto value = getFlowFromExpressionWithoutCopy(fieldSelect->expr, retroactive_field_selects);
    ...
    value.field_selects[value.field_selects.size() - 1].push_back(fieldSelect->field_index);

    return value;
  }
  auto deref = dynamic_cast<Dereference *>(&expr);
  if (deref != nullptr) {
    auto value = getFlowFromExpressionWithoutCopy(deref->expr, retroactive_field_selects);
  ...
  if (addr != nullptr) {
    auto value = getFlowFromExpressionWithoutCopy(addr->expr, retroactive_field_selects);
    ...
    field_selects_on_deref.insert(field_selects_on_deref.end(), lastSelects.begin(), lastSelects.end());
...
bool FlowValue::fieldsMatchOrContain(const FlowValue &value) const {
  if (indirect_level != value.indirect_level || expr.val.id != value.expr.val.id) {
    assert(false);
...
bool FlowValue::contains(const FlowValue &value) const {
  ...
  const FlowValue &newFlow =
      value.indirect_level > indirect_level ? withAddedIndirections(value.indirect_level - indirect_level) : *this;

  return newFlow.fieldsMatchOrContain(value);
}

FlowValue FlowValue::specializeFields(const FlowValue &src, const FlowValue &dst) const {
...
Flow::Flow(const FlowValue &from, const FlowValue &into) : from(from), into(into) {
...
bool Flow::containsFrom(const FlowValue &value) const { return from.contains(value); }

bool Flow::containsInto(const FlowValue &value) const { return into.contains(value); }
...
FuncFlowValue::FuncFlowValue(int32_t arg_index, int32_t indirect_level,
                             const std::vector<std::vector<int32_t>> &field_selects)
    : arg_index(arg_index), indirect_level(indirect_level), field_selects(field_selects) {
  assert(field_selects.size() == (uint32_t)(indirect_level + 1));
}

FuncFlow::FuncFlow(const FuncFlowValue &from, const FuncFlowValue &into) : from(from), into(into) {}

FuncFlowValue FuncFlowValue::fromFlowValue(const FlowValue &value,
...
FuncFlow FuncFlow::fromFlow(const Flow &flow, const std::vector<std::reference_wrapper<Allocation>> &args) {
  return FuncFlow(FuncFlowValue::fromFlowValue(flow.from, args), FuncFlowValue::fromFlowValue(flow.into, args));
}
...
FlowValue FuncFlowValue::toFlowValue(const std::vector<std::reference_wrapper<Expression>> &args,
                                     Expression &returnExpr, const FlowValue *escapeValue) {
  if (arg_index == -2) {
    assert(escapeValue != nullptr);
    /* copy escapeValue and set escape flag */
    return FlowValue(escapeValue->expr, escapeValue->indirect_level, escapeValue->field_selects, EscapeType::OTHER);
  }
  ...
  auto exprValue = FlowValue::getFlowFromExpressionWithoutCopy(expr, field_selects_on_deref);

  assert(exprValue.field_selects.size() == (uint32_t)(exprValue.indirect_level + 1));
  ...
  if (!field_selects.front().empty()) {
    ...
    exprSelectsBack.insert(exprSelectsBack.end(), field_selects.front().begin(), field_selects.front().end());
  }
  ...
  if (!field_selects_on_deref.empty()) {
    ...
    /* add field selects */
    exprValue.field_selects[0].insert(exprValue.field_selects[0].end(), field_selects_on_deref.begin(),
...
Flow FuncFlow::toFlow(const std::vector<std::reference_wrapper<Expression>> &args, Expression &returnExpr) {
...
template <class T> static bool vectorContains(const std::vector<T> &vector, const T &value) {
  return std::find(vector.begin(), vector.end(), value) != vector.end();
}

void traceFlow(const FlowValue &value, const FlowValue &initValue, const FlowList &flows,
               const std::function<void(const FlowValue &)> &func,
               const std::function<void(const FlowValue &)> &specializedFlowFunc, TraceFlowVisitedState &state) {
  for (auto &flow : flows) {
...
void traceValueBackwards(const FlowList &flows, const std::function<void(const FlowValue &)> &func,
  ...
  for (auto &flow : flows) {
    ...
    if (useFlow) {
      ...
      const Flow &newFlow = flow.into.contains(value) ? flow.specializeForInto(value) : flow;
...
void traceEscapesBackwards(const FlowList &flows, const std::function<void(const FlowValue &)> &func,
...
int EscapeAnalysisPass::visitFunctionDeclare(FunctionDeclare &instruct, const EscapeAnalysisState &state) {
  ...
  if (print_escapes) {
    visitAllocations(instruct.body, [this](Allocation &alloc) {
      if (AllocationTypeIsHeap(alloc.allocType)) {
        output << "%" << scopedNameToString(alloc.val.id.sourceName->getFullyScopedName()) << " escapes\n";
      }
    });
  }
  ...
  for (auto &arg : instruct.argAllocs) {
    ...
    calculateFunctionFlowMetadata(instruct.flow_metadata, instruct.argAllocs, flows, value, trace_state);
  }
...
void EscapeAnalysisPass::markEscapingAllocations(const ir::FlowList &flows, const BasicBlockList &blocks) {
  ...
  traceEscapesBackwards(
      ...
      [&blocks](const FlowValue &value) {
        ...
        visitAllocations(blocks, [&value](Allocation &alloc) {
          ...
          if (alloc.val.id == value.expr.val.id && !AllocationTypeIsHeap(alloc.allocType)) {
...
bool EscapeAnalysisPass::argsContain(const std::vector<std::reference_wrapper<Allocation>> &funcArgs,
...
void EscapeAnalysisPass::calculateFunctionFlowMetadata(ir::FuncFlowList &functionFlows,
  ...
  traceFlow(
      ...
      [&functionFlows, &funcArgs, &srcValue](const FlowValue &dstValue) {
        ...
        if (argsContain(funcArgs, srcValue.expr) &&
            (EscapeTypeIsEscape(dstValue.is_escape) || argsContain(funcArgs, dstValue.expr))) {
          ...
          if (srcValue.indirect_level < 1) {
            ...
            auto newSrc = FlowValue(srcValue.expr, srcValue.indirect_level + 1, newSelects, srcValue.is_escape);
      ...
      [this, &functionFlows, &funcArgs, &flows, &trace_state](const FlowValue &specialValue) {
        /* trace specialized value */
        calculateFunctionFlowMetadata(functionFlows, funcArgs, flows, specialValue, trace_state);
      },
      trace_state);
}

int EscapeAnalysisPass::visitBasicBlock(BasicBlock &instruct, const EscapeAnalysisState &state) {
...
int EscapeAnalysisPass::visitAllocation(Allocation &instruct, const EscapeAnalysisState &state) {
...
int EscapeAnalysisPass::visitFunctionCall(FunctionCall &instruct, const EscapeAnalysisState &state) {
  ...
  if (instruct.function.hasFlowMetadata()) {
    ...
    instruct.function.addFlowMetadata(*state.curFlowList, instruct.arguments, instruct);
  }
...
int EscapeAnalysisPass::visitStore(Store &instruct, const EscapeAnalysisState &state) {
...
int EscapeAnalysisPass::visitAddress(Address &instruct, const EscapeAnalysisState &state) {
  ...
  if (alloc != nullptr &&
      (alloc->allocType == AllocationType::ARG || alloc->allocType == AllocationType::UNRESOLVED_FUNC_ARG)) {
...
int EscapeAnalysisPass::visitTupleLiteral(TupleLiteral &instruct, const EscapeAnalysisState &state) {
...
int EscapeAnalysisPass::visitReturn(Return &instruct, const EscapeAnalysisState &state) {
...
int EscapeAnalysisPass::visitBuiltinCast(BuiltinCast &instruct, const EscapeAnalysisState &state) {
...
int EscapeAnalysisPass::visitForwardIdentifier(ForwardIdentifier &instruct, const EscapeAnalysisState &state) {
...
void runEscapeAnalysis(const ir::InstructionList &ir, bool print_flows, bool print_escapes,
                       bool print_func_flow_metadata, std::ostream &output) {
  auto pass = EscapeAnalysisPass(print_flows, print_escapes, print_func_flow_metadata, output);
